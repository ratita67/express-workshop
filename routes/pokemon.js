...
pokemon.post("/", async(req,res,next)=>{
    ...
    if(rows.affectedRows ==1){
        return res.status(201).json({code:201, message:"Pokemon insertado correctamene"});
    }
...
pokemon.put("/:id([0-9]{1,3})", async (req, res, next) =>{
    ...
    const rows = await db.query(query);
    if(rows.affectedRows ==1){
        return res.status(200).json({code:200, message:"Pokemon ctualizado correctamene"});
    }
    return res.status(200).json({code: 500, message: "Ocurrió un error"}); 
    }
    return res.status(500).json ({code:500, message: "Campos incompletos"})
...
pokemon.patch("/:id([0-9]{1,3})", async (req, res, next) =>{
    ...
    if(rows.affectedRows ==1){
        ...
        return res.status(500).json({code:(500), essage:"Campos incompletos" })
});
